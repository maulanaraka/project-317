...
@section('content')
    ...
    <div class="w-1/2 m-auto border-2 border-black p-32">
        ...
        <h1 class="text-center">From Event Update Organizer</h1>
        ...
        <form action="/organizer/updateEvent" method="POST" enctype="multipart/form-data">
            @csrf
            @method('put')
            ...
            <div class="w-1/2 m-auto space-y-4">
                ...
                <select name="event_category" id="cars">
                    @foreach ($category as $cl)
                        :
                        <option value="{{ $cl->id }}" {{ $myEvent->event_category == $cl->id ? 'selected' : '' }}>
